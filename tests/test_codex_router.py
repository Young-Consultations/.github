...
def test_portfolio_tasks_dispatch_command_matches_workflow_interface(monkeypatch):
    ...
    monkeypatch.setenv(
        "WORKFLOW_REF",
        f"{repository}/.github/workflows/codex-execute.yml@codex-adapter-v2.3.2",
    )
    monkeypatch.delenv("GITHUB_OUTPUT", raising=False)
    commands = []
    def fake_run(cmd, **kwargs):
...
def test_dispatch_rejects_invalid_execution_without_running_gh(monkeypatch):
    ...
    monkeypatch.setenv(
        "WORKFLOW_REF",
        "Young-Consultations/portfolio-tasks/.github/workflows/codex-execute.yml@codex-adapter-v2.3.2",
    )
    monkeypatch.delenv("GITHUB_OUTPUT", raising=False)
    monkeypatch.setattr(
