...
def validate(root: Path = ROOT, *, require_publishable: bool = False) -> list[str]:
    ...
    runtime_path = root / str(manifest.get("current_runtime", ""))
    credential_roles_path = root / str(manifest.get("credential_role_manifest", ""))
    ...
    if not credential_roles_path.is_file():
