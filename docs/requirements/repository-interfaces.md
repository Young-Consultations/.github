...
| RI-MVP-02 `.github` target adapter | Bounded execution target, isolated from the control plane. | Router-admitted canonical execution input only | Canonical result through RI-MVP-01 and, in implement mode only, one managed draft PR in `.github` | `Young-Consultations/.github/.github/workflows/codex-execute.yml@main` | Target-only identity cannot approve, route, modify another repository, merge, release, deploy, or use control-plane credentials. |
