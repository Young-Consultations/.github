...
| --- | --- | --- | --- | --- | --- |
| Router / `.github` control plane | `.github/workflows/codex-router.yml` | `task_payload`, `execution_mode`, `correlation_id` | `CODEX_ROUTER_TOKEN` | `accepted`, `target_repository`, `workflow_ref`, `execution_payload`, `failure_category`, `diagnostic_summary` | Admit only the task rules above and an enabled exact registry entry. |
| Target adapter / each selected repository | `.github/workflows/codex-execute.yml` in that target, referenced by its registry entry | `execution_payload` containing the complete canonical input | Target-owned executor credential; exact local name is owned and documented by that target | Canonical result delivered to the receiver | Must validate again; target credentials cannot route, approve, write another repository, merge, release, or deploy. |
