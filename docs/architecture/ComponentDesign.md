...
| Result Interpreter | Validate identity/status/evidence and determine authoritative state | Target result → accepted progress/terminal decision | Contract authority, evidence resolver | Invalid/inconsistent result quarantined; scale per delivery |
...
| Compatibility Verifier | Prove interface, permissions, modes, invalid cases, idempotency before enablement | Registration/release → signed or durable report | Read-only target access/test harness | Failure blocks enablement; parallel by target, rate limited |
